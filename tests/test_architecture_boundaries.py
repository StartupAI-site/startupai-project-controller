...
def test_consumer_review_queue_wiring_routes_group_processing_through_module() -> None:
    imported = _controller_runtime_imports(SRC_ROOT / "consumer_review_queue_wiring.py")
    assert (
        "startupai_controller.consumer_review_queue_group_wiring" in imported
    ), "consumer_review_queue_wiring.py should depend on consumer_review_queue_group_wiring"
    source = _source_text(SRC_ROOT / "consumer_review_queue_wiring.py")
    forbidden_defs = [
    ...
    offending = [token for token in forbidden_defs if token in source]
    assert offending == [], (
        "consumer_review_queue_wiring.py still defines the inline due-group "
        f"processing surface: {offending}"
    )
...
def test_consumer_support_wiring_routes_runtime_control_helpers_through_module() -> (
    ...
    assert "startupai_controller.consumer_resolution_support_wiring" in imported, (
        "consumer_support_wiring.py should depend on "
        "consumer_resolution_support_wiring"
    )
    offending = sorted(
        module
        for module in imported
        ...
        in {
            ...
            "startupai_controller.board_graph",
            "startupai_controller.consumer_resolution_helpers",
            "startupai_controller.consumer_selection_retry_wiring",

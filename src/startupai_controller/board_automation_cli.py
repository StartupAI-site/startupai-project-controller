...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_done = subparsers.add_parser(
        "mark-done", help="PR merge -> mark linked issues Done"
    )
    ...
    p_promote.add_argument(
        "--issue", required=True, help="Issue ref of the Done issue, e.g. crew#88"
    )
    p_promote.add_argument(
        "--this-repo-prefix",
        required=True,
    ...
    p_admit.add_argument(
        "--issue", help="Optional issue ref filter for targeted diagnosis"
    )
    p_admit.add_argument(
        "--limit", type=int, help="Optional cap for candidate/skip lists in output"
    )
    ...
    p_schedule = subparsers.add_parser(
        "schedule-ready", help="Classify Ready issues; optional claim mode"
    )
    schedule_scope = p_schedule.add_mutually_exclusive_group(required=True)
    schedule_scope.add_argument(
        "--this-repo-prefix", help="Prefix for the current repo"
    )
    schedule_scope.add_argument(
        "--all-prefixes",
        action="store_true",
    ...
    p_claim = subparsers.add_parser(
        "claim-ready", help="Explicitly claim one Ready issue into In Progress"
    )
    ...
    claim_target.add_argument(
        "--issue", help="Claim a specific issue ref (e.g. crew#88)"
    )
    claim_scope = p_claim.add_mutually_exclusive_group(required=False)
    claim_scope.add_argument(
        "--this-repo-prefix", help="Limit --next candidate selection to one prefix"
    )
    claim_scope.add_argument(
        "--all-prefixes",
        action="store_true",
    ...
    p_enforce = subparsers.add_parser(
        "enforce-ready-dependencies", help="Block Ready issues with unmet predecessors"
    )
    ...
    p_audit.add_argument(
        "--max-age-hours", type=int, default=24, help="Staleness threshold in hours"
    )
    _add_dry_run_argument(p_audit)
    p_audit.set_defaults(func=core._cmd_audit_in_progress)

    p_sync = subparsers.add_parser(
        "sync-review-state", help="Sync board state with PR/review/check events"
    )
    ...
    p_codex_gate = subparsers.add_parser(
        "codex-review-gate", help="Enforce codex-review verdict contract on PRs"
    )
    p_codex_gate.add_argument(
        "--pr-repo", required=True, help="PR repository (owner/repo)"
    )
    ...
    p_review_rescue_all = subparsers.add_parser(
        "review-rescue-all", help="Scan governed repos and reconcile PRs in Review"
    )
